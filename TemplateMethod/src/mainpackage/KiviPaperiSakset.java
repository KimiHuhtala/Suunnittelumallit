package mainpackage;

public class KiviPaperiSakset extends Peli {

    private enum Hands {
        ROCK, PAPER, SCISSOR
    }

    private int players;
    private Hands p1, p2;
    private boolean isDone = false;

    @Override
    protected void initGame(int playerCount) {
        System.out.println("Valitsit kivi, paperi, sakset pelin!");
        players = playerCount;
    }

    @Override
    protected void play(int player) {
        System.out.println("Pelaaja " + player + " valitse:");

        char select;
        System.out.println("\n\t\t\t1. Kivi");
        System.out.println("\t\t\t2. Paperi");
        System.out.println("\t\t\t3. Sakset");
        System.out.print("\n\n");
        select = Lue.character();
        switch (select) {
            case '1':
                System.out.println("Pelaaja " + player + " valitse " + Hands.ROCK.toString().toLowerCase());
                if(player == 1)
                    p1 = Hands.ROCK;
                else
                    p2 = Hands.ROCK;
                break;
            case '2':
                System.out.println("Pelaaja " + player + " valitse " + Hands.PAPER.toString().toLowerCase());
                if(player == 1)
                    p1 = Hands.PAPER;
                else
                    p2 = Hands.PAPER;
                break;
            case '3':
                System.out.println("Player " + player + " chose " + Hands.SCISSOR.toString().toLowerCase());
                if(player == 1)
                    p1 = Hands.SCISSOR;
                else
                    p2 = Hands.SCISSOR;
                break;
        }
        if(player == players){
            isDone = true;
        }
    }

    @Override
    protected boolean endGame() {
        return isDone;
    }

    @Override
    protected void printWinner() {
        switch (p1){
            case ROCK:
                switch (p2){
                    case ROCK:
                        System.out.println("Tasapeli");
                        break;
                    case PAPER:
                        System.out.println("Pelaaja 2 voittaa!");
                        break;
                    case SCISSOR:
                        System.out.println("Pelaaja 1 voittaa!");
                        break;
                }
                break;
            case PAPER:
                switch (p2){
                    case ROCK:
                        System.out.println("Pelaaja 1 voittaa!");
                        break;
                    case PAPER:
                        System.out.println("Tasapeli");
                        break;
                    case SCISSOR:
                        System.out.println("Pelaaja 2 voittaa!");
                        break;
                }
                break;
            case SCISSOR:
                switch (p2){
                    case ROCK:
                        System.out.println("Pelaaja 2 voittaa!");
                        break;
                    case PAPER:
                        System.out.println("Pelaaja 1 voittaa!");
                        break;
                    case SCISSOR:
                        System.out.println("Tasapeli");
                        break;
                }
                break;
        }
    }
}
